...
#testing
def hit(name, hand):
    hit = int(input(f"{name}'s turn ({sum(hand)}): \nTo hit (1) \nTo stand (2):"))
    while hit == 1:
        hitCard(hand)
        print("-"*30)
        if checkDeath(name, sum(hand)) == "loss": #if player goes over 21, they lose. calls function to check
            print("-"*30)
...
#hit(playerTwoName, playerTwoHand)
#hit(singlePlayerName, singlePlayerHand)
...
while mode != 4:
    if mode == 1:
        ...
        while hit(singlePlayerName, singlePlayerHand) == "stand":
